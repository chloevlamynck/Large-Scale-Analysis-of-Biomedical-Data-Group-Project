...
# Convert all columns (except the first three (ID, Protein.names, Symbol)) to numeric
...
Standardisation
```{r}
# Standardise the log-transformed abundance data
...
# View the first few rows of the standardised data
head(stand_log_abundance_only)

# Check if the standardisation was performed correctly
...
# Convert the numeric dataset to a matrix if it is not already
abundance_FilteringDone_clean_matrix <- as.matrix(abundance_FilteringDone_clean_numeric)

# Create a heatmap
pheatmap(
  abundance_FilteringDone_clean_matrix,
  main = "Heatmap (Before Standardisation and Log Transformation)",
  cluster_rows = TRUE,  # Enable hierarchical clustering of rows
  cluster_cols = TRUE,  # Enable hierarchical clustering of columns
  color = colorRampPalette(c("blue", "white", "red"))(100),  # Color scale from blue to red
  scale = "none",  # No scaling; use raw values
  show_rownames = FALSE,  # Hide row names for clarity
  show_colnames = FALSE   # Hide column names for clarity
)

# Convert the standardized vector to a matrix, if not already done
stand_log_abundance_only_matrix <- matrix(
  stand_log_abundance_only,
  nrow = 959,  # Replace with the actual number of rows in your data
  ncol = 102   # Replace with the actual number of columns in your data
)

# Create a heatmap
pheatmap(
  stand_log_abundance_only_matrix,
  main = "Heatmap (After Standardisation and Log Transformation)",
  ...
  scale = "none",  # No further scaling; data is already standardized
  show_rownames = FALSE,  # Hide row names for a cleaner plot
...
```{r}
# Standardise the log-transformed abundance data (mean = 0, sd = 1)
abundances_P_stand <- scale(abundances_P_Filtered_log)

# View the standardised data (first few rows)
head(abundances_P_stand)

# Check if the standardisation was correctly performed by verifying the mean and standard deviation of each column
...
# Transpose the standardised non-tumor tissue data
...
# Initialise a dataframe to store results from the Dunn's post-hoc test
